    ...
    >
      ...
      <div className="flex sm:hidden flex-col gap-4 p-6">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-full overflow-hidden shrink-0">
            <img src={assetSrc(item.image)} alt={item.name} className="gsapImage w-full h-full object-cover" />
          </div>
          <div>
      ...
      <div className="hidden sm:flex flex-row h-full">
        ...
        <div className="flex-[0_0_38%] overflow-hidden rounded-r-2xl">
          {item.image ? (
            <img src={assetSrc(item.image)} alt={item.name} className="gsapImage w-full h-full object-cover object-center" />
          ) : (
