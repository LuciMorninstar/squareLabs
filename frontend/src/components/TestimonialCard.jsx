import CardDots from "../assets/testimonial/CardDots.svg?react";
...
<div className="flex sm:hidden flex-col gap-4 p-6">
  <div className="flex items-center gap-4">
    ...
    <div>
      <span className="text-base font-bold font-sora text-black">{item.name}</span>
      <p className="text-xs font-sora text-gray-500">{item.position} - {item.company}</p>
    </div>
  </div>
  <p className="text-xs font-sora text-text-quarternary-color leading-6 line-clamp-7">
    "{item.testimonial}"
  </p>
</div>
      ...
      <div className="hidden sm:flex flex-row h-full">
        <div className="flex flex-col justify-between flex-1 p-8 xl:p-16">
          <CardDots className="w-6 h-6 md:w-8 md:h-8 text-white shrink-0" />
          <p className="lg:pt-6 xl:pt-8 text-sm md:text-base lg:text-md xl:text-lg text-text-quarternary-color leading-7 my-4 flex-1 line-clamp-6">
            {item.testimonial}
          </p>
          <div className="flex flex-col gap-1 shrink-0">
            <span className="text-sm sm:text-base lg:text-lg text-black font-bold font-sora">
              {item.name}
            </span>
